...
if 'final_inhibition_measurements' not in locals():
    final_inhibition_measurements = []
    print("Warning: 'final_inhibition_measurements' not found. Please ensure all previous cells, especially the 'Inhibition Zone Calculation' cell, have been run.")

df_measurements = pd.DataFrame(final_inhibition_measurements)

print("Inhibition Zone Radii (mm):")

if not df_measurements.empty:
    display_columns = ['Disk_ID', 'Min_Inhibition_Radius_mm', 'Max_Inhibition_Radius_mm']
    ...
    print("No inhibition zone measurements available to display.")
    json_measurements = json.dumps([], indent=4, ensure_ascii=False)
    print(json_measurements)